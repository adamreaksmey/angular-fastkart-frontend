...
export class RomeComponent {
  ...
  @Select(ProductState.categoryProducts) categoryProduct$: Observable<
    Product[]
  >;
  ...
  ngOnInit() {
    if (this.data?.slug == this.slug) {
      ...
      const getCatProducts$ = this.store.dispatch(
        new GetCategoryProducts({
          category_ids:
            this.data?.content?.categories_products?.category_ids?.join(","),
        }),
      );
    ...
    this.route.queryParams.subscribe((params) => {
      if (
        this.route.snapshot.data["data"].theme_option.productBox === "digital"
      ) {
        if (
