...
export class CollectionPriceFilterComponent {
  ...
  ngOnChanges() {
    this.selectedPrices = this.filter["price"]
      ? this.filter["price"].split(",")
      : [];
  }

  applyFilter(event: Event) {
    const index = this.selectedPrices.indexOf(
    ...
    this.router.navigate([], {
      ...
      queryParams: {
        price: this.selectedPrices.length
          ? this.selectedPrices?.join(",")
          : null,
        page: 1,
      },
